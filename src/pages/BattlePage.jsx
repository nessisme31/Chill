...
export default function BattlePage({ battle, onPause, onConfig, onClose }) {
  ...
  const changeTab = (t) => {
    setTab(t)
    localStorage.setItem(`citc_tab_${battle.id}`, t)
  }
  ...
  const handleClose = async () => {
    ...
    const championName = slot?.team_name || null
  ...
  const sharedProps = { battle, judges, djs, crews, setCrews }
    ...
    <div className="page">
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.85)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200, padding: 16 }}>
          <div className="card" style={{ maxWidth: 380, width: '100%', textAlign: 'center', padding: '36px 28px' }}>
            ...
            <div className="muted" style={{ marginBottom: 28 }}>
              Le battle passera en archives. Le champion sera affiché sur la page d'accueil. Cette action est irréversible.
            </div>
      ...
      <div className="flex-between" style={{ marginBottom: 20 }}>
        ...
        <div className="flex" style={{ gap: 8 }}>
          <button className="btn btn-ghost btn-sm" onClick={onConfig}>
            ⚙ Configuration
          </button>
          <button
